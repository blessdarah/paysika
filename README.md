...
Domain events (`TransferCompleted`, `DepositFailed`, etc.) are published synchronously after successful operations. The notification service subscribes to events and sends emails either synchronously (tests, `EMAIL_BACKGROUND=false`) or via RQ (production, `EMAIL_BACKGROUND=true`).
...
1. `POST /api/v1/deposits` → creates PENDING transaction, returns 202
2. Provider sends webhook to `POST /api/v1/payments/webhook`
3. `deposit.completed` → `confirm_deposit()` creates ledger entries, marks COMPLETED
4. `deposit.failed` → marks FAILED without entries
...
| **Synchronous email in dev** | No Redis/worker needed to run locally | Email blocks the request; toggle via `EMAIL_BACKGROUND=true` |
...
| **Two-phase deposit** | Provider owns the payment lifecycle; platform never creates entries without confirmation | More complex state machine; partial failures possible |
...
| `EMAIL_BACKGROUND` | `False` | Enqueue email jobs via RQ |
